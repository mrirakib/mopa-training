   ...
   <div class="row justify-content-center">
      <div class="col-md-12">
         <div class="card">
            ...
            <div class="card-body">
               <form name="user-institute" action="/entry-user-approval-authority/{{$userMapping->id}}" method ="POST" enctype="multipart/form-data">
                  {{ csrf_field() }}
                  {{ method_field('PUT') }}
                  <div class="row">
                     <div class="form-group col-12">
                        ...
                        <select name="entry_user_id" class="form-control form-control-sm" required onchange="getApprovalAuthorityInfo(this.value)">
                           ...
                           <option value="{{$userMapping->entry_user_id}}" selected>{{$userMapping->getEntryUserInfo->name}}-{{$userMapping->getEntryUserInfo->section}}</option>
                        </select>
                     </div>
                     <div class="form-group col-12">
                        ...
                        <select name="approval_authority_user_id" id="approval_authority_user_id" class="form-control form-control-sm" required>
                           ...
                           <option value="{{$rowdata->id}}" @if($rowdata->id == $userMapping->approval_authority_user_id) selected @endif>{{$rowdata->name}}-{{$rowdata->section}}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
